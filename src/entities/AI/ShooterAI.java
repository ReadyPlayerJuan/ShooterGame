...
public class ShooterAI extends AI {
	private final float radius1 = 100f;
	private final float radius2 = 150f;
	private final float radius3 = 250f;
	private final float newPositionTimerMax = 0.5f;
	private float newPositionTimer = 0;
	
	private int strafeDirection = -1;
	private final boolean predictPlayerPosition = true;
	
	public ShooterAI(Entity owner) {
		super(owner);
	}
		...
		if(newPositionTimer < 0) {
			...
			if(nearestPlayerIndex != -1) {
				...
				if(distance > radius3) {
					...
					if(Math.random() < 0.3 * newPositionTimerMax) {
						strafeDirection *= -1;
					}

					float moveDistance = speed * newPositionTimer;
					float angle = directionToPlayer + ((float)Math.random() * 0.6f - 0.3f) + (3.14159f / 2 * strafeDirection);
					...
					float moveDistance = speed * newPositionTimer * -1;
					float angle = directionToPlayer + ((float)Math.random() * 1.8f - 0.9f);
					targetPosition = Vector2f.add(owner.getPosition(), new Vector2f(moveDistance * (float)Math.cos(angle), moveDistance * (float)Math.sin(angle)), null);
				}
